const {
  Etablissement,
  validateCreateEtablissement,
  validateUpdateEtablissement,
...
const create = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({
        message: `Établissement '${newEtablissement.nom}' créé avec succès`,
      });
  } catch (error) {
    console.log(error);
...
const update = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: `Établissement ${etablissement.nom} modifié avec succès`,
      });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: error.message });
